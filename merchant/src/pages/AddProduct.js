  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const formattedImages = product.images.map((image, index) => {
        if (typeof image === 'string') {
          // Image is just a filename/ID from upload
          return {
            filename: image,
            originalName: image,
            path: `/uploads/${image}`,
          ...
          // Image is already an object (from existing product)
          return {
            ...image,
            isPrimary: index === 0,
          };
            ...
            <div className='uploaded-images'>
              {product.images.map((image, index) => {
                ...
                const imageKey =
                  typeof image === 'string'
                    ? image
                    : image._id || image.filename || index;
                const imageSrc = (() => {
                  if (typeof image === 'string') {
                    // Manual upload - image is just an ID/filename
                    return `/api/uploads/${image}`;
                  } else if (image.path) {
                    // Image is an object with path
                    const path = image.path;

                    // If it's an external URL, use as-is
                    if (path.startsWith('http')) return path;

                    // If path starts with /uploads/, extract the filename/ID
                    if (path.startsWith('/uploads/')) {
                      const fileId = path.replace('/uploads/', '');
                      return `/api/uploads/${fileId}`;
                    }

                    // Otherwise, prepend /api if needed
                ...
                return (
                  <div key={imageKey} className='image-preview'>
                    ...
                    <button
                      ...
                      onClick={() =>
                        handleRemoveImage(
                          typeof image === 'string' ? image : imageKey
                        )
                      }
                    >
                      ×
