...
const Support = () => {
  ...
  const handleStatusUpdate = async (ticketId, newStatus) => {
    try {
      await axios.patch(`/support/${ticketId}/status`, { status: newStatus });
      fetchTickets();
    } catch (error) {
  ...
  const handlePriorityUpdate = async (ticketId, newPriority) => {
    try {
      await axios.patch(`/support/${ticketId}/status`, { priority: newPriority });
      fetchTickets();
    } catch (error) {
  ...
  const handleAddResponse = async (e) => {
    ...
    try {
      await axios.post(`/support/${selectedTicket._id}/response`, {
        message: responseMessage
      });
      setResponseMessage('');
      // Refresh ticket details
      const response = await axios.get(`/support/${selectedTicket._id}`);
  ...
  const getPriorityBadge = (priority) => {
    ...
    const config = priorityConfig[priority] || { class: 'priority-medium', label: priority };
  ...
  return (
    <div className="support-page">
      <div className="page-header">
        ...
        <p className="page-subtitle">Manage customer and merchant support requests</p>
      </div>
      ...
      <div className="filters-section">
        ...
        <div className="filter-group search-group">
          ...
          <input
            type="text"
            placeholder="Ticket number, name, email..."
                ...
                <tr key={ticket._id}>
                  ...
                  <td>
                    <button
                      ...
                      onClick={async () => {
                        try {
                          const response = await axios.get(`/support/${ticket._id}`);
                          setSelectedTicket(response.data);
                        } catch (error) {
            ...
            <div className="modal-body">
              <div className="ticket-info">
                ...
                {selectedTicket.orderNumber && (
                  <div className="info-row">
                    <strong>Order:</strong> {selectedTicket.orderNumber}
                  </div>
                )}
                <div className="info-row">
